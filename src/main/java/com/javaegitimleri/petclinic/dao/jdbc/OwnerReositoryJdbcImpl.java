...
@Repository
public class OwnerReositoryJdbcImpl implements OwnerRepository {

    @Autowired
    private JdbcTemplate jdbcTemplate;

    private RowMapper<Owner> ownerRowMapper = new RowMapper<Owner>() {
        @Override
    ...
    public Owner findById(Long id) {
        ...
        return DataAccessUtils.singleResult(jdbcTemplate.query(sql, ownerRowMapper, id));
    }

    @Override
    public List<Owner> findByLastName(String lastName) {
        String sql = "select id,first_name,last_name from t_owner where last_name like ?";
        return jdbcTemplate.query(sql, ownerRowMapper, "%" + lastName + "%");
    }
